...
| Inline case-study cards | `components/chat/CaseStudyIntroCard.tsx`, `CaseStudyIntroDeck.tsx` | Round 25: replaced the standalone case-study grid - the 3 real case studies render inside the chat's opening instead. Round 26 (full pivot to a real reference site) stripped the card chrome entirely: no border, no background box, no shadow, no rounded container - just a full-bleed image, a tiny uppercase tag, and a bold title underneath, in a plain equal-width 3-up grid (the earlier "salon hang," AWS bigger than the other two, is gone - the reference uses one equal grid). Clicking one opens `CaseStudyModal`. | Styling | Keep the per-project visual (real Qlik screenshot, AWS 67/100, Sprout swatches) honest - same rule as everywhere else, no fabricated numbers or screens. |
